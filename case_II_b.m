/* We work on the case II subcase b*/


function case_II_b( : bound := 20, NewE2s := [], NewF2s := [])
	/*
	INPUT:
		- ``NewE21s`` : the space of newforms of level 2^8 * 5 * 7 as Hilbert newforsm.
		- ``NewF21s`` : the space of newforms of level 2^7*3*5^2*7 as Hilbert newforms.
	...
	*/
	...
	if #NewF2s eq 0 then
		NewF2s := Hilbert_newforms(2^7 * 3 * 5^2 * 7);
	end if;
				...
				for y1 in [0..q-1] do
					for a in [0..q-2] do
						for b in [0..q-2] do
							...
							// F21 Frey curve
							AF2 := 10 * (3 * z1* 5^(2*b-1) + 2);
							BF2 := 70;

							// The discriminant of F21
			...
			if #[dif: dif in Ppair | dif ne 0] ne 0 then
				...
				if #Pr gt 0 and Max(Pr) gt 5 then
					...
					if more_qs le 100 then
						elimination_pair := case_II_b( : bound := more_qs, NewE2s := [*E2new*], NewF2s := [*F2new*]);
					else
						return false;
					end if;
