/* We work on the case I subcase c*/


function case_I_c( : bound := 20, NewE1s := [], NewF1s := [])
	/*
	INPUT:
		- ``NewE1s`` : the space of newforms of level 2^8 * 5^2 * 7 as Hilbert newforms.
		- ``NewF1s`` : the space of newforms of level 2^3 * 3 * 5 * 7 as Hilbert newforms.
	...
	*/
	...
	if #NewF1s eq 0 then
		NewF1s := Hilbert_newforms(2^3 * 3 * 5 * 7);
	end if;
			...
			if #[dif: dif in Ppair | dif ne 0] ne 0 then
				...
				if #Pr gt 0 and Max(Pr) gt 5 then
					...
					if more_qs le 100 then
						elimination_pair := case_I_c( : bound := more_qs, NewE1s := [*E1new*], NewF1s := [*F1new*]);
					else
						return false;
					end if;
