...
function case_IV_a( : bound := 20, NewE4s := [], NewF4s := [])
	/*
	INPUT:
		- ``NewE4s`` : the space of newforms of level 2 * 5 * 7 as Hilbert newforms.
				...
				for y1 in [0..q-1] do
					for a in [0..q-2] do
						for b in [0..q-2] do
							...
							//Frey curve for E1
							AE4 := 2^(2*a-2) * 5^(2*b) * z1;
							BE4 :=  7 * 2^(4*a - 7) * 5^(4*b - 1) * z1^2;

							// The discriminant of E1
							DE4 :=  BE4^2 * (-16*AE4^2 - 8*AE4 + 64*BE4 - 1);
							...
							if DF4 ne 0 and DE4 ne 0 then
								...
								aqE4 := TraceOfFrobenius(EllipticCurve([1, AE4,0,BE4,0]),q);
			...
			if #[dif: dif in Ppair | dif ne 0] ne 0 then
				...
				if #Pr gt 0 and Max(Pr) gt 5 then
					...
					if more_qs le 100 then
						elimination_pair := case_IV_a( : bound := more_qs, NewE4s := [*E4new*], NewF4s := [*F4new*]);
					else
						return false;
					end if;
