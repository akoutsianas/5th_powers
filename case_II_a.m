/* We work on the case II subcase a*/


function case_II_a( : bound := 20, NewE2s := [], NewF2s := [])
	/*
	INPUT:
		- ``NewE2s`` : the space of newforms of level 2^8 * 5 * 7 as Hilbert newforms.
		- ``NewF2s`` : the space of newforms of level 2 * 3 * 5^2 * 7 as Hilbert newforms.
	...
	*/
	...
	if #NewF2s eq 0 then
		NewF2s := Hilbert_newforms(2 * 3 * 5^2 * 7);
	end if;
				...
				for y1 in [0..q-1] do
					for a in [0..q-2] do
						for b in [0..q-2] do
							...
							// F2 Frey curve
							AF2 :=  - (3*z1* 5^(2*b) + 5 * 2^(2*a + 1)  + 1)/4;
							BF2 := 35 * 2^(4*a) /2^5;

							// The discriminant of F2
							DF2 := -16*AF2^2 - 8*AF2 + 64*BF2 - 1;


							//Frey curve for E2
							AE2 := 4 * (z1* 5^(2*b) + 2^(2*a));
							BE2 :=  14* 5^(4*b-1)*z1^2;
							...
							if DF2 ne 0 and DE2 ne 0 then
								aqF2 := TraceOfFrobenius(EllipticCurve([1, AF2,0,BF2,0]),q);
			...
			if #[dif: dif in Ppair | dif ne 0] ne 0 then
				...
				if #Pr gt 0 and Max(Pr) gt 5 then
					...
					if more_qs le 100 then
						elimination_pair := case_II_a( : bound := more_qs, NewE2s := [*E2new*], NewF2s := [*F2new*]);
					else
						return false;
					end if;
