/* We work on the case I subcase a*/


function case_I_a( : bound := 20, NewE1s := [], NewF1s := [])
	/*
	INPUT:
		- ``NewE1s`` : the space of newforms of level 2^8 * 5^2 * 7 as Hilbert newforms.
		- ``NewF1s`` : the space of newforms of level 2 * 3 * 5 * 7 as Hilbert newforms.
	...
	*/
	...
	if #NewF1s eq 0 then
		NewF1s := Hilbert_newforms(2 * 3 * 5 * 7);
	end if;
				...
				for y1 in [0..q-1] do
					for a in [0..q-2] do
						for b in [0..q-2] do
							...
							// F1 Frey curve
							AF1 :=  - (3*z1 + 2^(2*a + 1) * 5^(2*b + 1) + 1)/4;
							BF1 := 7 * 2^(4*a) * 5^(4*b+1)/2^5;

							// The discriminant of F1
							DF1 := -16*AF1^2 - 8*AF1 + 64*BF1 - 1;


							//Frey curve for E1
							AE1 := 20 * (z1 + 2^(2*a) * 5^(2*b));
							BE1 :=  70*z1^2;
							...
							if DF1 ne 0 and DE1 ne 0 then
								aqF1 := TraceOfFrobenius(EllipticCurve([1, AF1,0,BF1,0]),q);
			...
			if #[dif: dif in Ppair | dif ne 0] ne 0 then
				...
				if #Pr gt 0 and Max(Pr) gt 5 then
					...
					if more_qs le 100 then
						elimination_pair := case_I_a( : bound := more_qs, NewE1s := [*E1new*], NewF1s := [*F1new*]);
					else
						return false;
					end if;
